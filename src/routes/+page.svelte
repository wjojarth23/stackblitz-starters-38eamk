<script>
  ...
  async function handlePost(activityDetails) {
    ...
    console.log(
      ...
      new Date(
        value.year,
        value.month - 1, // Months are zero-indexed in JavaScript (January is 0)
        value.day,
        /* optional hours, minutes, seconds, milliseconds */
      ),
    );
    ...
    unsubscribe = await pb
      ...
      .subscribe("*", async ({ action, record }) => {
        ...
        if (action === "update") {
          ...
          const existingIndex = allFlexes.findIndex((m) => m.id === record.id);
          if (existingIndex !== -1) {
            allFlexes[existingIndex] = record; // Update the record at the found index
          } else {
            console.warn(`Record with id ${record.id} not found for update`);
          }
        }
      });
  });
...
<div class="flex p-4 gap-2">
  {#each allActivities as activity, index}
    <Popover.Root>
      <Card.Root>
        <Card.Header>{activity.name}</Card.Header><Card.Content
          ><div>{activity.maxStudents}</div>
          <div>Max Students</div></Card.Content
        ><Card.Footer
          ><Popover.Trigger
            class="bg-primary text-primary-foreground hover:bg-primary/90 p-1 w-full rounded-md"
            >Post Session</Popover.Trigger
          ></Card.Footer
        ></Card.Root
      >
      <br />
  ...
  {#each allFlexes as flex, index}
    <Popover.Root>
      <Card.Root>
        <Card.Header
          >{allActivities.find((entry) => entry.id === flex.activity)
            .name}</Card.Header
        >
        <Card.Content>
          <div class="flex gap-2 justify-center flex-col">
            ...
            <Popover.Trigger
              class="bg-primary text-primary-foreground hover:bg-primary/90 p-1 w-full rounded-md"
              >Refer Students</Popover.Trigger
            >
            <Popover.Content>
              <Command.Root>
                  ...
                  {#each allUsers.filter((obj) => obj.role === "student") as student, index}
                    <Command.Item
                      onSelect={(currentValue) => {
                        referrStudent(flex, student, index);
                      }}>{student.name}</Command.Item
                    >
                  {/each}
                </Command.List>
              </Command.Root>
